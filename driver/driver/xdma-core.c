...
static int is_config_bar(struct xdma_dev *lro, int idx)
{
	...
	dbg_init("irq_id = 0x%llx, cfg_id = 0x%llx\n", irq_id, cfg_id);
...
//	while (result[engine->rx_head].status) {
	while (engine->rx_head != engine->rx_tail) { //ymc
	...
	else if (eop)
	{
		...
		for(ii = head; ii != engine->rx_head; ii = (ii+1)%RX_BUF_PAGES)//ymc
		{
			result[ii].status = 0;
			result[ii].length = 0;
		}
	...
	{
		engine->rx_head = head;
		dbg_tfr("rx_head reaches rx_tail and No EOP get, roll back rx_head and return 0\n");
	}

...
static int test_dma(char *devicename, uint32_t addr, uint32_t size, uint32_t offset, uint32_t count, char *filename)
{
  ...
  posix_memalign((void **)&allocated, 4096/*alignment*/, size );
  assert(allocated);
  ...
  printf("CLOCK_MONOTONIC reports %ld.%09ld seconds (total) for %d transfers of %d bytes\n", ts_end.tv_sec, ts_end.tv_nsec, i, rc);

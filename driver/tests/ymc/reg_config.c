...
int main(int argc, char **argv) {
  ...
  off_t i = 0;
  /* access width */
  char *device = "/dev/xdma0_user";

    if ((dev = open(device, O_RDWR | O_SYNC)) == -1) FATAL;
    printf("character device %s opened.\n", device ); 
    fflush(stdout);

    /* map one page */
    map_base = mmap(0, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev, 0);
...
// test mailbox
    ...
    target = 0x0000;
    addr = map_base + target;

  fp = fopen("./test_bench/reg.txt","r");
  perror("Open");

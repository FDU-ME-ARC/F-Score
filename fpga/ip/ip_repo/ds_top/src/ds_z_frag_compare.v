...
reg     [4 : 0]    mz_spec_num;
assign mz_data_mask_r = (frag_len[4 : 0] == 5'h0) ? 6'h20 : (6'h20 - {1'b0, frag_len[4 : 0]});

assign pep_frame_deep = (frag_len[4 : 0] == 5'h0) ? ((frag_len >> 5) - 5'h1) : frag_len >> 5; 
...
wire    [71 : 0]                    pep_mz_value_i[0 : 63];
reg     [19 : 0]                    pep_mz_value[0 : 63];
wire    [31 : 0]                    m_proton;
wire    [31 : 0]                    m_hydrogen;
...
assign     m_hydrogen = ds_ctrl[1] ? `M_HYDROGEN_1 : `M_HYDROGEN_0;
...
generate
  ...
  //      assign pep_mz_value_i[2*i] = (frag_mz_tmp[i * 40 +: 40] * coef_1_r) + (m_proton * WOE);
  //      assign pep_mz_value_i[2*i+1] = (frag_mz_tmp[i * 40 +: 40] * coef_1_r) + (m_hydrogen * coef_1_r) + (m_proton * WOE);
  //      assign spec_mz_value[i]     =  (|spec_mz_tmp[20 * i +: 20]) ? spec_mz_tmp[20 * i +: 20] : 20'h6ffff;
...
reg    [19 : 0]    pep_mz_value_align_0;
reg    [19 : 0]    pep_mz_value_align_1;
...
wire  [63 : 0]  coef_2_r;
wire  [63 : 0]  coef_3_r;
wire  [63 : 0]  pep_mz_value_q [0 : 63];
generate
    genvar ii;
        for (ii = 0; ii < 32; ii = ii + 1) begin
            mult_40_32 mult_pep_i(.CLK(clk), .A(frag_mz_tmp[ii*40 +: 40]), .B(coef_1_r), .CE(pep_new), .P(pep_mz_value_i[ii]));
            assign pep_mz_value_q[2 * ii] = pep_mz_value_i[ii][63 : 0] + coef_2_r;
            assign pep_mz_value_q[2 * ii + 1] = pep_mz_value_i[ii][63 : 0] + coef_2_r + coef_3_r;
        end
endgenerate

mult_32_32 coef_gen_2(.CLK(clk), .A(m_proton), .B(WOE), .CE(pep_new), .P(coef_2_r));
mult_32_32 coef_gen_3(.CLK(clk), .A(m_hydrogen), .B(coef_1_r), .CE(pep_new), .P(coef_3_r));


always @(posedge clk or posedge rst) begin
    if (rst) begin
        for (j = 0; j < 32; j = j + 1) begin
            pep_mz_value[2*j] <= 20'h0;
            pep_mz_value[2*j+1] <= 20'h0;
        end
    end
    else if (pep_new_r) begin
        for (j = 1; j < 32; j = j + 1) begin
            pep_mz_value[2 * j]   <= ((mz_frame_num == 5'h0) & (j <= mz_data_mask_r)) ? 20'h0 : pep_mz_value_q[2*(j-1)][59 : 40];
            pep_mz_value[2 * j + 1]   <= ((mz_frame_num == 5'h0) & (j <= mz_data_mask_r)) ? 20'h0 : pep_mz_value_q[2 * (j-1) + 1][59 : 40];
        end
            pep_mz_value[0] = pep_mz_value_align_0;
            pep_mz_value[1] = pep_mz_value_align_1;
    end
    else begin
        for (j = 0; j < 32; j = j + 1) begin
            pep_mz_value[2*j] <= pep_mz_value[2*j];
            pep_mz_value[2*j+1] <= pep_mz_value[2*j + 1];
        end        
    end
end
...
localparam  PEP_LIMIT = 64/CMP_STEP - 1;
...
generate
    ...
    for (jj = 0; jj < CMP_STEP; jj = jj + 1)begin
        ...
        assign p_p_tmp [jj]  = pep_p_value[(pep_pointer << SHIFT_BIT-1) + (jj >> 1)];
    end
endgenerate
...
always @(posedge clk or posedge rst) begin
    if (rst) begin
        ...
        pep_mz_value_align_0 <= 20'h0;
        pep_mz_value_align_1 <= 20'h0;
        ...
        read_frag_addr <= 5'b0;
           ...
           //need to be defined because the c type frag
        ...
        if (p_mz_tmp[0] > s_mz_tmp[CMP_STEP-1]) begin
            ...
            if (pep_end) begin
                ...
                pep_pointer <= 5'b0;
                frag_mz_tmp <= frag_xt_mz;
                pep_mz_value_align_0 <= pep_mz_value_q[62][59 : 40];
                pep_mz_value_align_1 <= pep_mz_value_q[63][59 : 40];
                coef_1_r <= coef_1[31 : 0];
                frag_p_tmp  <= frag_xt_p;
                read_frag_addr <= read_frag_addr + 5'b1;
                mz_frame_num <= frag_bram_addr;
...
//wire match_calc;
//assign match_calc = current == COMP_STAT && (~state_next) && (~state_end) && (~pep_new) && (pep_mz_value[pep_pointer] == spec_mz_value[spec_pointer]);

//mult_40_8 ds_x_calc(.CLK(clk), .A(spec_i_value[spec_pointer]), .B(pep_p_value[{1'b0, pep_pointer[5 : 1]}]), .CE(match_calc), .P(ds_x_tmp));
...
//assign match_hit = ((current == COMP_STAT) && (~state_next) & (~state_end) & (~pep_new) & (~pep_new_r)) ? ((|cmp_res[0]) | (|cmp_res[1]) | (|cmp_res[2]) | (|cmp_res[3])) : 1'b0; 
...
//assign  ds_x_tmp = (ds_x_tmp_0[match_index_q[0]] + ds_x_tmp_1[match_index_q[1]]) + (ds_x_tmp_2[match_index_q[2]] + ds_x_tmp_3[match_index_q[3]]);
...
//assign  pep_next = (current == COMP_STAT) & (pep_pointer == 6'b0) & (~state_next);
//    
//    
